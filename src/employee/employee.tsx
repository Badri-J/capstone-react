...
const EmployeePage: React.FC = () => {
  ...
  const [rout, setRout] = useState<any[]>([]);
  ...
  const filterItems = () => {
    setFilteredItems(
      rout.filter((routItem: any) =>
        routItem.name.toLowerCase().includes(searchTerm.toLowerCase())
      )
    );
  ...
  return (
    <div className="homepage">
      ...
      <h1 className="heading_emp">Employee Details</h1>
      <input
        value={searchTerm}
        onChange={(e) => {
      ...
      <div>
        <table className="tableemp tableemp-striped bdr">
          <thead>
            <tr>
          ...
          <tbody>
            {filteredItems.map((k: any, index: number) => (
              <tr key={index} >
                {/* <td>{index + 1}</td> */}
                <td>{k.empid}</td>
                <td>
